...
export function OnboardingLogros() {
  const navigate = useNavigate();
  const { updateOnboarding } = useAuthStore();

  const opciones = [
    'Primeras 24 horas completadas',
    'Alcanzar una semana',
    'Logro del primer mes',
    'Otro',
  ];

  const handleSelect = (logro: string) => {
    updateOnboarding({ logroEsperado: logro });
    navigate('/onboarding/recordatorio');
  };
    ...
    <div className="min-h-screen bg-[#1a1b2e] flex items-center justify-center p-6">
      <div className="max-w-md w-full space-y-8 text-center">
        ...
        <h1 className="text-xl font-bold text-white">
          ¿Qué logro esperas con mas ganas?
        </h1>
